...
public class RestaurantDetailsActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private List<String> clientListOfTheDay;
    ...
    private void configureRecyclerview() {
        Log.e(TAG, "configureRecyclerview + clientListOfTheDay = " + clientListOfTheDay);
        if (clientListOfTheDay != null) {
            adapter = new ClientListAdapter(clientListOfTheDay, Glide.with(recyclerView));
    ...
    private void getRestaurantDetails() {
        if (restaurantId != null) {
            ...
            showSnackBar(constraintLayout, getString(R.string.details_restaurant_not_restaurant));
        }
    }
                ...
                if (documentSnapshot.exists()) {
                    ...
                    if (usersToday != null) {
                        ...
                        if (dateRegistered.equals(today)) {
                            ...
                            RestaurantFirebase.updateClientsTodayList(listUsersToday, id).addOnSuccessListener(new OnSuccessListener<Void>() {
                                @Override
                            ...
                            updateUserTodayInFirebase(userId, id, new OnSuccessListener<Void>(){
                                @Override
                                public void onSuccess(Void unused) {
                                    getClientList();
                                }
                    ...
                    RestaurantFirebase.createRestaurant(id, name, restaurantAddress);
                  //  Log.e(TAG, "RestaurantFirebase.createRestaurant");
    ...
    private void updateUserTodayInFirebase(String myId, String myRestoId, OnSuccessListener<Void> onSuccessListener) {
        List<String> listUsersToday = new ArrayList<>();
        listUsersToday.add(myId);
        RestaurantFirebase.updateClientsTodayList(listUsersToday, myRestoId).addOnSuccessListener(onSuccessListener);
    }
    ...
    private void updateLikeRestaurantInFirebase(final String idResto) {
        UserFirebase.getUser(userId).addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
            ...
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                if (documentSnapshot.exists()) {
                    listRestaurantsLike = Objects.requireNonNull(documentSnapshot.toObject(User.class)).getRestaurantsLike();
    ...
    private void updateTheRestaurantOfTheDayView(String id) {
        ...
        UserFirebase.getUser(userId).addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
            @Override
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                restaurantOfTheDay = Objects.requireNonNull(documentSnapshot.toObject(User.class)).getRestaurantOfTheDay();
                lastRestaurantDate = Objects.requireNonNull(documentSnapshot.toObject(User.class)).getRestaurantChoiceDate();

                if (restaurantOfTheDay != null && restaurantOfTheDay.length() > 0 && lastRestaurantDate.equals(today)) { // We check that there is a restaurant registered and that it was registered today
                    if (restaurantOfTheDay.equals(idToday)) {
                        choiceButton.setImageResource(R.drawable.ic_go_restaurant);
                    }
                }
            }
